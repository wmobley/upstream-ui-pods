...
interface RemovePodPermissionInput {
  podId: string;
  user: string;
}

const useRemovePodPermission = () => {
  const { basePath, token } = usePodsConfig();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ podId, user }: RemovePodPermissionInput) => {
      ...
      try {
        return await api.deletePodPermission({ podId, user });
      } catch (error) {
        throw await normalizePodsApiError(error, 'Unable to remove permission');
      }
    },
...
export default useRemovePodPermission;
